...
##### Advanced settings
# Source IP filtering
## 192.168.15.0/24 = OpenVPN network
declare -a sourcesIpAllowed=("217.182.168.213","192.168.15.0/24")
...
function firewallSetup {
    log_daemon_msg "Firewall initialization"
        ...
        # IPv6
        # ----------------------------------------------------
        allowIpv6LAN "fd42:feed:feed:feed::/64"
...
function forwardConfiguration {
    log_daemon_msg "Forward rules"
        ...
        ### IPv4
        declare -a forwardIpAllowed=("217.182.168.213" "172.16.100.0/24" "192.168.15.0/24")
        # Allow forward for specific sources
        for forwardIP in "${forwardIpAllowed[@]}"
        do
            #echo " Allow forwarding for IPv4: $forwardIP"
            allowForwardingFromIpv4 $forwardIP $forwardIP
        done
        # Forward rules
        DAXIONGMAO_SERVER="217.182.168.213"
        #echo " Allow port forwarding IPv4, $DAXIONGMAO_SERVER to .."
...
function incomingPortFiltering {
    ...
    #inputFiltering tcp 56700 "Custom SSH" true
...
function outgoingPortFiltering {
    ...
    outputFiltering tcp 56700 "Custom SSH"
...
###### VPN
vpn "tun0" 8080 udp "ens3" "192.168.15.0/24" "fd42:feed:feed:feed::/64"
#allowForwardingFromIpv4 192.168.15.0/24 "VPN lan"
